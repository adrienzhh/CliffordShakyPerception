...
class Mover:
    ...
    def mover(self):
        ...
        target_x = 4.0 # m for distance travel 
        ...
        while not rospy.is_shutdown():
            ...
            if not self.is_move:
                ...
                self.vel_msg.angular.z = 1.0 * angle_error  # Angular velocity for turning
            ...
            if self.is_move:
                self.vel_msg.linear.x = 1  # Linear velocity for forward movement
                self.vel_msg.angular.z = 0.0  # Angular velocity (zero for straight motion)
                if self.pos_y > target_x:
                    self.is_move_2 = False
            
            if not self.is_move_2:
                angle_error = np.radians(180) - self.angle
                self.vel_msg.linear.x = 0.0  # Stop forward movement
                self.vel_msg.angular.z = 1.0 * angle_error  # Angular velocity for turning
                if abs(self.angle - np.radians(180)) < 0.1:
                    self.is_move_3 = True
            ...
            if self.is_move_3:
                ...
                self.vel_msg.angular.z = 0.0  # Angular velocity (zero for straight motion)
                if self.pos_x < 1:
                    self.is_move_4 = False
            

            if not self.is_move_4:
                angle_error = np.radians(-90) - self.angle
                self.vel_msg.linear.x = 0.0  # Stop forward movement
                self.vel_msg.angular.z = 1.0 * angle_error  # Angular velocity for turning
                if abs(self.angle - np.radians(-90)) < 0.1:
                    self.is_move_5 = True
